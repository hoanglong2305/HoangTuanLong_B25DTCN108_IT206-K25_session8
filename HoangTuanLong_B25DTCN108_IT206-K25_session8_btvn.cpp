...
void inputStudents(struct Student students[], int count) {
    ...
    for (int i = 0; i < count; i++) {
        ...
        getchar(); // Xóa ký t? xu?ng dòng còn l?i trong buffer
        
        printf("Nhap ho ten: ");
        fgets(students[i].name, sizeof(students[i].name), stdin);
        // Xóa ký t? xu?ng dòng ? cu?i chu?i n?u có
...
int readFromFile(struct Student students[], int maxCount) {
    ...
    // Ð?m b?o không d?c quá maxCount
    if (count > maxCount) {
        count = maxCount;
    }
    ...
    for (int i = 0; i < count; i++) {
        ...
        // B? qua ký t? xu?ng dòng còn l?i sau khi d?c s?
        fgetc(file);
        // Ð?c tên (có th? ch?a kho?ng tr?ng)
